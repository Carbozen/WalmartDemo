...
// In a real application, this would be an actual fetch request to your backend API.
async function fetchWalmartStoresData() {
  try {
    // Dynamically import the JSON file.
    // Ensure the path '../../data/walmartStores.json' is correct relative to this component.
    const response = await import('../../walmartStores.json');
    return response.default; // Access the default export for JSON data
  } catch (error) {
...
const ManagerDashboardPage = () => {
  ...
  // State to hold all store data fetched from the JSON file
  const [allStores, setAllStores] = useState([]);
  // State for the currently selected store, which will be displayed in the side drawer
  const [selectedStore, setSelectedStore] = useState(null);
  // State to control the visibility of the side drawer
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);
  // State to control the filter type applied to the map markers ('all', 'best', 'worst')
  const [filterType, setFilterType] = useState('all');

  // Define color palette, consistent with your previous components
  const colors = {
  ...
  // useEffect hook to fetch store data when the component mounts
  useEffect(() => {
    fetchWalmartStoresData().then(data => {
      setAllStores(data); // Set the fetched data to state
    });
  }, []); // Empty dependency array means this effect runs only once after initial render

  // useMemo hook to filter and sort stores based on the selected filterType.
  // This prevents re-filtering on every render if dependencies haven't changed.
  const filteredStores = useMemo(() => {
    // Create a mutable copy of the stores array for sorting
    const storesCopy = [...allStores];

    if (filterType === 'best') {
      // For 'best' performing, sort by carbon footprint in ascending order (lower CO2e is better)
      // and take the top 5 stores.
      return storesCopy.sort((a, b) => a.performance.carbonFootprint.current - b.performance.carbonFootprint.current).slice(0, 5);
    } else if (filterType === 'worst') {
      // For 'worst' performing, sort by carbon footprint in descending order (higher CO2e is worse)
      // and take the top 5 stores.
      return storesCopy.sort((a, b) => b.performance.carbonFootprint.current - a.performance.carbonFootprint.current).slice(0, 5);
    }
    // If filterType is 'all' or any other value, return all stores
    return storesCopy;
  }, [allStores, filterType]); // Dependencies: re-run when 'allStores' or 'filterType' changes

  // Handler for when a store marker on the map is clicked
  const handleStoreClick = (store) => {
    setSelectedStore(store); // Set the clicked store as the selected store
    setIsDrawerOpen(true); // Open the side drawer
  };

  // Handler for closing the side drawer
  const handleDrawerClose = () => {
    setIsDrawerOpen(false); // Close the side drawer
    setSelectedStore(null); // Clear the selected store data
  };
  ...
  return (
    ...
    >
      <div className="layout-container flex h-full grow flex-col">
        {/* Header section - remains consistent across pages */}
        ...
        <div className={`px-10 flex flex-1 flex-col py-5 transition-all duration-300 ease-in-out ${isDrawerOpen ? 'pr-96' : ''}`}>
          ...
          <div className="flex flex-wrap gap-2 p-4">
            <a
              ...
              onClick={() => router.push('/dashboard')} // Navigate back to a general dashboard
            >
              Home
            </a>
          ...
          <div className="flex gap-3 p-3 flex-wrap">
            <button
              ...
              className={`flex h-10 shrink-0 items-center justify-center rounded-lg px-4 text-sm font-medium leading-normal transition-colors duration-200 ${
                filterType === 'all' ? 'bg-[#51946b] text-white' : 'bg-[#e8f2ec] text-[#0e1a13] hover:bg-[#d1e6d9]'
              }`}
            >
              Show All Stores
            </button>
            <button
              ...
              className={`flex h-10 shrink-0 items-center justify-center rounded-lg px-4 text-sm font-medium leading-normal transition-colors duration-200 ${
                filterType === 'best' ? 'bg-green-600 text-white' : 'bg-[#e8f2ec] text-[#0e1a13] hover:bg-[#d1e6d9]'
              }`}
            >
            ...
            <button
              ...
              className={`flex h-10 shrink-0 items-center justify-center rounded-lg px-4 text-sm font-medium leading-normal transition-colors duration-200 ${
                filterType === 'worst' ? 'bg-red-600 text-white' : 'bg-[#e8f2ec] text-[#0e1a13] hover:bg-[#d1e6d9]'
              }`}
            >
              Worst Performing (Carbon)
            </button>
          </div>

          {/* Map Area */}
          <div className="flex-1 p-4 min-h-[400px]"> {/* Ensures map has a minimum height */}
            {/* Conditionally render map or loading message */}
            {allStores.length > 0 ? (
              <IndiaMap stores={filteredStores} onStoreClick={handleStoreClick} filterType={filterType} />
            ) : (
      ...
      {/* Side Drawer Component (conditionally rendered) */}
