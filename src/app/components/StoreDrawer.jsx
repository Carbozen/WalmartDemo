...
const StoreDrawer = ({ isOpen, onClose, store }) => {
  ...
  const getCarbonPerformanceColorClass = (current, target) => {
    ...
    } else if (current <= target * 1.1) { // 10% over target
  ...
  const getWastePerformanceColorClass = (current, target) => {
    ...
    if (current >= target) {
      return 'text-green-600'; // Meeting or exceeding target (good)
    } else if (current >= target * 0.9) { // 10% below target
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#e8f2ec] rounded-lg p-4 mb-6 shadow-sm">
        <h3 className="text-xl font-semibold mb-2 text-[#0e1a13]">Carbon Footprint</h3>
        <p className="text-lg mb-1">
          Current: <span className={`${getCarbonPerformanceColorClass(store.performance.carbonFootprint.current, store.performance.carbonFootprint.target)} font-bold`}>
            {store.performance.carbonFootprint.current} {store.performance.carbonFootprint.unit}
          </span>
        </p>
        <p className="text-sm text-gray-600">Target: {store.performance.carbonFootprint.target} {store.performance.carbonFootprint.unit}</p>
        <p className="text-sm text-gray-600">Last Period: {store.performance.carbonFootprint.lastPeriod} {store.performance.carbonFootprint.unit}</p>
      </div>

      {/* Other Key Metrics */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-[#e8f2ec] rounded-lg p-4 shadow-sm">
          <h3 className="text-lg font-semibold mb-2 text-[#0e1a13]">Energy Consumption</h3>
          <p className="text-lg">
            {store.performance.energyConsumption.current} {store.performance.energyConsumption.unit}
          </p>
        </div>
        <div className="bg-[#e8f2ec] rounded-lg p-4 shadow-sm">
          <h3 className="text-lg font-semibold mb-2 text-[#0e1a13]">Waste Diversion</h3>
          <p className="text-lg">
            <span className={`${getWastePerformanceColorClass(store.performance.wasteDiversion.current, store.performance.wasteDiversion.target)} font-bold`}>
      ...
      {store.issues && store.issues.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6 shadow-sm">
          <h3 className="text-xl font-semibold mb-2 text-red-700">🚨 Problems Faced:</h3>
          <ul className="list-disc list-inside text-gray-700 space-y-1">
      ...
      {store.suggestions && store.suggestions.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 shadow-sm">
          <h3 className="text-xl font-semibold mb-2 text-green-700">💡 Suggestions for Improvement:</h3>
          <ul className="list-disc list-inside text-gray-700 space-y-1">
      ...
      <p className="text-xs text-gray-500 mt-4 text-center">Data updated periodically for performance tracking.</p>
    </div>
  );
};
